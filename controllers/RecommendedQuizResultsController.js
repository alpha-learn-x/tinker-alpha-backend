...
exports.getUserQuizResults = async (req, res) => {
    try {
        const { quizName, userId } = req.params;

        if (!quizName || !userId) {
            return res.status(400).json({ message: 'quizName and userId are required' });
        }

        // Find all quiz results by userId and quizName
        const results = await QuizResult.find({ quizName, userId });

        if (!results || results.length === 0) {
            return res.status(404).json({ message: 'No quiz results found for the given user and quiz name' });
        }

        // Extract totalMarks for each result
        const totalMarksList = results.map(result => ({
        ...
        // Sum total marks
        const totalMarksSum = totalMarksList.reduce((sum, entry) => sum + entry.totalMarks, 0);
